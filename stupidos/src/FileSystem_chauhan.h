...
private:
  ...
  LinkedList<int> blocks; // Linked list of block indexes, aka contents of the file
...
public:
  ...
  std::vector<int> getFileBlocks() const; // Returns the indexes that make up the file
};
...
public:
  ...
  FileManager(int hardDriveSize);           // Default constructor with hard drive size parameter
  FileManager(const std::string &filename); // Default constructor with filename parameter

  // Methods
  void addFile(const std::string &name, const std::string &contents); // Adds a file to the hard drive
  void deleteFile(const std::string &name);                           // Deletes a file from the hard drive
  std::string readFile(const std::string &name);                      // Reads a file from the hard drive
  std::vector<std::string> getFileNames();                            // Returns the names of all the files on the hard drive
